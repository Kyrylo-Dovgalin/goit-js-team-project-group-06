...
function clickTrailer(event){
    ...
    fetchTrailer(id,1).then(data=>{console.log(data.data.results[0].key),window.open(`https://www.youtube.com/watch?v=${data.data.results[0].key}`, '_blank'); 
    if (data.data.results.length==0) {Notify.failure(`Oops, no trailer for this film`);trailerButton.disabled=true;
    }}
